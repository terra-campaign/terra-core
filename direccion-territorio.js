...
async function reload(){const g=++generation;$('workspace').hidden=true;$('refresh').disabled=true;$('clear').disabled=true;$('status').textContent='Consultando actividad…';try{
 ...
 $('workspace').hidden=false;$('status').textContent='Actualizado: '+new Date(d.calculatedAt).toLocaleString('es-MX');
...
onAuthStateChanged(auth,user=>{generation++;$('workspace').hidden=true;$('session').textContent='';if(!user){location.replace('./login.html');return;}reload();});
