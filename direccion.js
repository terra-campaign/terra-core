...
onAuthStateChanged(auth,async user=>{const version=++epoch;$('municipalities').replaceChildren();$('setup').hidden=true;$('leaderMissions').hidden=true;$('refresh').disabled=true;
 ...
 try{const p=(await getDoc(doc(db,'usuarios',user.uid))).data();if(version!==epoch)return;if(!p||p.active!==true||!['admin','lider_principal'].includes(p.role))throw Error('Acceso reservado al Líder principal y administrador.');$('setup').hidden=p.role!=='admin';$('leaderMissions').hidden=p.role!=='lider_principal';await refresh();}catch(e){$('status').textContent=e.message;}
});
